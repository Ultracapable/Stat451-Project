...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel("Plot 2: Gender and Race Impact",
         plotOutput("plot2"),
         p("This plot examines the impact of gender and race on MBA admission probabilities, using log-odds coefficients from a logistic regression model. Log-odds coefficients represent the logarithm of the odds of admission associated with each variable, where positive coefficients indicate higher odds and negative coefficients indicate lower odds. The results show that female applicants are more likely to be admitted than males, suggesting a gender-based differential. Among racial groups, 'Other,' White, and Asian applicants show higher admission probabilities, while Hispanic and Black applicants face lower admission rates, with Black applicants having the lowest likelihood of admission. These patterns point to potential systemic inequities or biases in the admissions process."),
         p("To analyze this, one-hot encoding was applied to categorical variables like gender and race, transforming each category into binary columns (e.g., Male = 1, Female = 0). This method ensures that categories are treated independently without implying an ordinal relationship and enables logistic regression to process the data numerically. One-hot encoding allows the model to isolate the independent effects of each demographic factor, highlighting disparities in admission probabilities and providing insights into systemic biases within the admissions process.Under this case, both genderMale and raceWhite are omitted because the model treats them as redundant due to the reference category handling in the encoding process.")
  ),
